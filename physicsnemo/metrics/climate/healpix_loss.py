...
class WeightedLandMSE( th.nn.MSELoss ):
    """
    Land MSE class offers impementaion for MSE loss with: 
    1) weighted by a land-sea-mask field. 
    2) weighted by channel
    """
    def __init__(
        self,
        lsm_file: str,
    ...
    def setup(self, trainer):
        ...
        # lsm gives the percentage of pixel that has ocean
        self.lsm_tensor = th.tensor(np.expand_dims(self.lsm_ds.values,(0,2,3))).to(trainer.device)
    ...
    def forward(self, prediction, target, average_channels=True ):
        ...
        if not self.lsm_sum_calculated:
            ...
            self.lsm_sum_calculated = True
        # average weighted 
        land_err = (((target-prediction)**2)*self.lsm_tensor)
        land_mean_err = land_err.sum(dim=(0, 1, 2, 4, 5))
        land_mean_err = land_mean_err*self.loss_weights
        
        if average_channels: 
            return th.sum(land_mean_err)/self.lsm_sum
        else: 
            return land_mean_err/self.lsm_var_sum
